...
Алгоритм сортировки выбором заключается в поиске минимального элемента в оставшейся неотсортированной части массива и его помещении на соответствующую позицию в отсортированную часть.
Каждое прохождение внешним циклом определяет минимальный элемент и помещает его на начало неотсортированного участка.
Процесс продолжается до тех пор, пока весь массив не будет отсортирован.

**Временная сложность:**
 
Временная сложность алгоритма сортировки выбором в худшем, среднем и лучшем случаях равна O(n^2).
...
**Внешний цикл** (for i in range(n - 1)) выполняется n-1 раз, где n - длина массива

**Внутренний цикл** (for j in range(n - i - 1)) на каждой итерации внешнего цикла проходит по неотсортированной части массива

**Сравнение и обмен:** на каждом шаге внутреннего цикла сравниваются соседние элементы arr[j] и arr[j + 1]

**"Всплытие" максимума:** после каждой итерации внешнего цикла самый большой элемент из неотсортированной части "всплывает" в конец массива
...
Ключевой элемент: key = array[i] - текущий элемент, который нужно вставить в правильную позицию

Внутренний цикл (while j >= 0 and array[j] > key):

Сдвигает элементы, большие чем key, на одну позицию вправо

Ищет правильную позицию для вставки key

Вставка: array[j + 1] = key - помещает ключевой элемент на найденную позицию

**Временная сложность:**

**O(n²)** в худшем случае

**O(n)** в лучшем случае (уже отсортированный массив)
...
Массив рекурсивно делится на две примерно равные части

Процесс продолжается до тех пор, пока не останутся массивы длиной 1 (базовый случай)

**Слияние (Merge):**

Отсортированные массивы объединяются в один отсортированный массив

На каждом шаге сравниваются первые элементы двух массивов и выбирается меньший

Создается результирующий массив result длиной left.length + right.length

**Три указателя:** i для left, j для right, k для result

**Сравнение элементов:** выбирается меньший элемент из двух массивов

**Добавление остатков:** оставшиеся элементы добавляются в конец
...
**Выбор промежутков (gaps):**

Начинаем с большого промежутка gap = n // 2

Постепенно уменьшаем промежуток в 2 раза: gap //= 2

**Сортировка с промежутками:**

Для каждого промежутка выполняем сортировку вставками, но сравниваем элементы на расстоянии gap

Элементы, отстоящие друг от друга на gap, образуют подмассивы

**Внешний цикл:** while gap > 0 - управляет последовательностью промежутков

**Средний цикл:** for i in range(gap, n) - проходит по всем элементам для текущего промежутка

**Внутренний цикл:** while j >= gap and arr[j - gap] > temp - выполняет вставку с шагом gap
